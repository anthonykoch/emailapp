...
class Login extends React.Component<Props, State> {
  ...
  onSubmit = async (e) => {
    ...
    if (username === '') {
      return this.setState({
        error: 'Your username can not be empty',
      })
    } else if (password == '') {
      return this.setState({
        ...
        first={
          <form>
            <LoginInner>
              ...
              {error && (
                <styles.spacing.Margin top="6" bottom="3">
                  <Error>{error}</Error>
                </styles.spacing.Margin>
              )}
  ...
  render() {
    const page =
      this.state.isAuthPrecheckFinished
        ? this.renderLogin()
        : null
